...
public class LibroRestController {
	...
	/*---Add new libro---*/
	@PostMapping("/libro")
	public ResponseEntity<?> save(@RequestBody Evento libro) {
	...
	public ResponseEntity<Evento> get(@PathVariable("id") long id) {
		Evento libro = libroService.getByIsbn13(id);
	...
	public ResponseEntity<List<Evento>> list() {
		List<Evento> libros = libroService.getAll();
		return ResponseEntity.ok().body(libros);
	}

	/*---Update a libro by id---*/
	@PutMapping("/libro/{id}")
	public ResponseEntity<?> update(@PathVariable("id") long id, @RequestBody Evento libro) {
